    ...
    def test_device_routes_require_signatures(self):
        ...
        for path in paths:
            with self.subTest(path=path):
                self.assertEqual(request_policy("POST", path), "device")

    def test_device_route_near_matches_still_require_dashboard_authentication(self):
        for path in (
            "/api/voice/clientish/test",
            "/api/camera-talk/clients/test",
            "/api/security/keyclient-session-extra",
        ):
            with self.subTest(path=path):
                self.assertEqual(request_policy("POST", path), "dashboard")
