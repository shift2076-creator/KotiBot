...
class DeviceNotificationCredentialTests(unittest.TestCase):
    ...
    def test_failed_new_token_write_is_retried_and_survives_reload(self):
        with TemporaryDirectory() as temp_dir:
            state_file = self._state_file(Path(temp_dir))
            store = DeviceNotificationCredentialStore(state_file)
            with patch.object(store, '_save_unlocked', side_effect=OSError('fixture disk failure')):
                with self.assertRaises(OSError):
                    store.set_token('key-1', 'fixture-token', 100)
            self.assertEqual(store.credential('key-1'), {})
            self.assertFalse(state_file.exists())
            store.set_token('key-1', 'fixture-token', 200)
            self.assertEqual(DeviceNotificationCredentialStore(state_file).credential('key-1'),
                             {'token': 'fixture-token', 'updated_at': 200.0})

    def test_failed_token_update_preserves_previous_record_until_retry(self):
        with TemporaryDirectory() as temp_dir:
            state_file = self._state_file(Path(temp_dir))
            store = DeviceNotificationCredentialStore(state_file)
            previous = store.set_token('key-1', 'fixture-before', 100)
            with patch.object(store, '_save_unlocked', side_effect=OSError('fixture disk failure')):
                with self.assertRaises(OSError):
                    store.set_token('key-1', 'fixture-after', 200)
            self.assertEqual(store.credential('key-1'), previous)
            self.assertEqual(DeviceNotificationCredentialStore(state_file).credential('key-1'), previous)
            store.set_token('key-1', 'fixture-after', 300)
            self.assertEqual(DeviceNotificationCredentialStore(state_file).credential('key-1')['token'], 'fixture-after')

    def test_failed_removal_preserves_record_and_retry_persists_revocation(self):
        with TemporaryDirectory() as temp_dir:
            state_file = self._state_file(Path(temp_dir))
            store = DeviceNotificationCredentialStore(state_file)
            previous = store.set_token('key-1', 'fixture-token', 100)
            with patch.object(store, '_save_unlocked', side_effect=OSError('fixture disk failure')):
                with self.assertRaises(OSError):
                    store.remove('key-1')
            self.assertEqual(store.credential('key-1'), previous)
